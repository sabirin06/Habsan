  ...
  try {
    if (typeof navigator !== "undefined" && (navigator as any).share) {
      await (navigator as any).share({
        title,
        text: "Check out this stay on TravelPro",
        url
      })
      return
    }
...
export function StayGalleryModal({
  open,
  onOpenChange,
  images,
  title
  ...
  return (
    <>
      {/* Use non-modal dialog so our full-screen viewer overlay (rendered outside the dialog portal)
          can receive pointer events. */}
      <Dialog open={open} onOpenChange={onOpenChange} modal={false}>
        <DialogContent
          className={[
            isMobile
              ? "left-0 top-0 translate-x-0 translate-y-0 w-screen h-dvh rounded-none max-h-none"
              : "max-h-[85vh]"
          ].join(" ")}
        >
          <DialogHeader className={isMobile ? "sticky top-0 bg-background/95 backdrop-blur-sm" : ""}>
            {/* `DialogContent` includes an absolute top-right close button.
                Reserve space so the Share button never overlaps it. */}
          ...
          <div
            className={isMobile ? "h-[calc(100dvh-64px)] overflow-auto" : "max-h-[calc(85vh-64px)] overflow-auto"}
          >
              ...
              {safe.map((img, i) => (
                ...
                >
                  <div className="aspect-4/3 bg-muted/30">
                    <img src={img.url} alt={img.alt} className="w-full h-full object-cover group-hover:scale-[1.02] transition-transform duration-300" />
                  </div>
      ...
      {/* In-modal viewer (no external libs) */}
      {viewerOpen ? (
        <div className="fixed inset-0 z-9999" role="dialog" aria-modal="true" aria-label="Photo viewer">
          ...
          {/* Content */}
          {/* Important: keep the full-screen container pointer-events-none so clicks outside the
              image/controls land on the backdrop (and close). */}
          <div className="relative z-10 h-full pointer-events-none">
            ...
            {/* Keep the full-screen positioning wrapper click-through; only the media/controls are interactive */}
